...
class TestErrorCodeRepository(unittest.TestCase):
    ...
    def test_upsert_fallback(self):
        ...
        with patch.object(self.repo, "_upsert_local") as mock_local:
            mock_local.return_value = ErrorCode(
                id="1",
                code="E100",
                severity="Crit",
                description=None,
    ...
    def test_load_fallback_success(self):
        ...
        with patch(
            "backend.infrastructure.repositories.error_code_repository.open",
            mock_open(read_data=mock_data),
        ):
            with patch(
                "backend.infrastructure.repositories.error_code_repository.Path.exists",
                return_value=True,
            ):
    ...
    @patch(
        ...
        read_data='[{"id": "1", "code": "E100", "severity": "Info"}]',
    )
    @patch(
        "backend.infrastructure.repositories.error_code_repository.Path.exists", return_value=True
    )
    def test_upsert_local_locked_existing(self, mock_exists, m_open):
        res = self.repo._upsert_local_locked(
            "E100",
            severity="Critical",
    ...
    def test_upsert_local_locked_new(self, mock_exists, m_open):
        res = self.repo._upsert_local_locked(
            "E200", severity="Warning", description=None, solution_url=None, solution_content=None
        )
