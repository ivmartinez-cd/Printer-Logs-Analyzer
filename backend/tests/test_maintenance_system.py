...
def mock_scheduler():
    with (
        patch("backend.application.scheduler.start_scheduler"),
...
def test_maintenance_repository_upsert_device(maintenance_repo):
    with (
        patch.object(maintenance_repo, "upsert_device") as mock_upsert,
...
def test_maintenance_repository_model_rules(maintenance_repo):
    family = "Test Family"
    with (
        patch.object(maintenance_repo, "upsert_model_rule") as mock_upsert,
        patch.object(maintenance_repo, "get_model_rules") as mock_get,
    ):
        rule = MaintenanceModelRule(
            model_family=family, component_type="Fuser", expected_life=100000, alert_margin=5000
        )
...
def test_maintenance_service_alert_trigger(
    mock_meters, mock_info, maintenance_service, maintenance_repo
):
    serial = "ALERT_TEST"
    family = "Test Family"
    ...
    # Mock repo calls
    with (
        patch.object(maintenance_repo, "get_model_rules") as mock_get_rules,
        ...
        mock_get_state.return_value = [
            MaintenanceDeviceState(
                device_serial=serial, component_type="Kit", last_change_counter=0
            )
        ]
...
@patch("backend.application.services.maintenance_service.get_device_info")
@patch("backend.application.services.maintenance_service.get_device_meters")
def test_maintenance_service_quiet_sync(
    mock_meters, mock_info, maintenance_service, maintenance_repo
):
    serial = "QUIET_TEST"
    family = "Test Family"
...
def test_maintenance_api_endpoints(client):
    ...
    with patch(
        "backend.infrastructure.repositories.maintenance_repository.MaintenanceRepository.get_all_devices"
    ) as mock_get:
