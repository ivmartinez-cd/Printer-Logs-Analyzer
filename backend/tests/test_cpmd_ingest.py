...
def _make_solution(model_family: str = _MODEL_FAMILY, cpmd_hash: str = _FAKE_HASH) -> ErrorSolution:
...
def test_second_call_with_same_hash_returns_skipped() -> None:
    ...
    with (
        ...
        report1 = ingest_cpmd([_MODEL_FAMILY], _FAKE_PDF, _API_KEY, repository=mock_repo)
    ...
    with (
        ...
        report2 = ingest_cpmd([_MODEL_FAMILY], _FAKE_PDF, _API_KEY, repository=mock_repo)
...
def test_different_hash_is_not_skipped() -> None:
    ...
    with (
        ...
        report = ingest_cpmd([_MODEL_FAMILY], other_pdf, _API_KEY, repository=mock_repo)
...
def test_all_high_confidence_no_llm_calls() -> None:
    ...
    with (
        ...
        report = ingest_cpmd([_MODEL_FAMILY], _FAKE_PDF, _API_KEY, repository=mock_repo)
...
def test_no_api_key_skips_llm_fallback() -> None:
    ...
    with (
        ...
        # No api_key supplied
        report = ingest_cpmd([_MODEL_FAMILY], _FAKE_PDF, api_key=None, repository=mock_repo)
...
def test_report_contains_correct_model_id_and_hash() -> None:
    """IngestReport must echo back model_family and cpmd_hash."""
    mock_repo = MagicMock()
    mock_repo.list_by_model.return_value = []
    ...
    assert report.model_families == [_MODEL_FAMILY]
