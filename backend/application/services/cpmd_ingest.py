...
class IngestReport:
    ...
    def model_family(self) -> str | None:
        """Back-compat: returns the first model_family in the list."""
        return self.model_families[0] if self.model_families else None
...
def ingest_cpmd(
    ...
    """Ingest a CPMD PDF for one or more printer model families.

    Idempotent: for each family in *model_families*, if solutions with the same
    PDF hash already exist in the repository, ingestion is skipped for
    that specific family. If all families are skipped, the function returns
    ...
    # --- Idempotence check per model family ---
    to_ingest: List[str] = []
    for family in model_families:
        existing = repo.list_by_model(family)
        if existing and any(s.cpmd_hash == cpmd_hash for s in existing):
            _logger.info(
                "[cpmd_ingest] model_family=%s hash=%s...: ya procesado, saltando",
                family,
                cpmd_hash[:12],
            )
            continue
        to_ingest.append(family)
    ...
    # --- Step 4: persist for each model family ---
    all_solutions: List[ErrorSolution] = []
    for family in to_ingest:
        m_solutions = _persist_batch(repo, family, pairs, cpmd_hash)
