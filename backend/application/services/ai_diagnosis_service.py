...
SYSTEM_PROMPT = (
    ...
    "- EVALUACIÓN DE URGENCIA (¿hay que mandar técnico ya?): Primero identificá el counter máximo del log completo (incluyendo eventos Info como 34.02.01 — NO solo errores). Ese es el counter real del último evento. Luego, para cada error crítico, calculá el delta = counter_max_log - counter_ultimo_error. Si ese delta es grande (ej. >200 páginas) significa que el equipo imprimió normalmente después del error y NO está en crisis activa ahora. Si el error fue en cluster (múltiples veces en pocas horas) pero el equipo siguió imprimiendo cientos de páginas sin repetirlo, la prioridad es media — la falla es real pero no es urgencia inmediata. Prioridad alta solo si: el error ocurre en el tramo final del log con delta < 50 páginas, O sigue en cluster hasta el cierre del log. Nunca confundas el último ERROR con el último EVENTO del log.\n"
