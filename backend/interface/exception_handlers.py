...
from backend.domain.exceptions import DomainError, ExternalServiceError, ResourceNotFoundError
...
def register_exception_handlers(app: FastAPI):
    ...
    async def domain_error_handler(request: Request, exc: Exception):
        ...
        return JSONResponse(
            status_code=status_code, content={"error": True, "code": code, "detail": detail}
        )
    ...
    async def http_exception_handler(request: Request, exc: StarletteHTTPException):
        return JSONResponse(
            ...
            content={"error": True, "code": f"HTTP_{exc.status_code}", "detail": exc.detail},
        )
    ...
    async def value_error_handler(request: Request, exc: ValueError):
        ...
        return JSONResponse(
            ...
            content={"error": True, "code": "VALUE_ERROR", "detail": detail},
        )
    ...
    async def general_exception_handler(request: Request, exc: Exception):
        if anthropic and isinstance(
            exc, (anthropic.APIError, anthropic.AuthenticationError, anthropic.RateLimitError)
        ):
            status_code = 502
