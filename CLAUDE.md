...
Componente principal (~970 líneas). Orquesta los sub-componentes y hooks; la lógica de análisis, modales, exportación PDF y filtrado de fecha viven en hooks separados. Las vistas `saved-list` y `saved-detail` y la barra de filtros de fecha viven en archivos separados.
...
Renderizado por el componente `DashboardHeader` — ver descripción en la tabla de componentes. DashboardPage pasa callbacks (`onOpenSavedList`, `onAnalyzeNew`, `onSaveIncident`, `onExportPdf`, `onHelp`) y props de estado (`hasResult`, `exportingPdf`, `logFileName`, `healthStatus`).
...
**Filtros y sorting:**
- Incidents: estado de filtro por severidad, búsqueda y sort viven **dentro de `IncidentsTable`** — DashboardPage solo pasa `incidentRows` (ya filtrados por fecha) y los callbacks de edición/solución.
- Events: estado de colapso, filtro por severidad, búsqueda y sort viven **dentro de `EventsTable`** — DashboardPage solo pasa `events={filteredEvents}` y `onViewSolution`.
...
**Helpers importantes:**
- `getIncidentTableRows(incidents, events, filter: DateFilter)` → `IncidentRow[]` — derivado de `result`, memoizado en DashboardPage
...
|------------|-----------|
| `DashboardHeader.tsx` | Header principal (logo SVG, logFileName, botones de acción, reloj, DbStatusBadge). Recibe props `logFileName`, `healthStatus`, `hasResult`, `exportingPdf` y callbacks de acción. "Exportar PDF" solo aparece cuando `hasResult`. `LiveClock` y `DbStatusBadge` son funciones locales del módulo. |
| `KPICards.tsx` | 4 cards de KPI: Estado de errores, Incidencias Activas, Último error crítico, Tasa de errores |
| `IncidentsTable.tsx` | Tabla de incidentes con expand/collapse de filas, sort, filtros y "ver más" en mensajes. Estado de filtro/sort es **interno** (no se levanta a DashboardPage). Recibe `incidentRows: IncidentRow[]` (ya filtrados por fecha), `onEditCode` y `onViewSolution`. Exporta tipo `IncidentRow`. |
| `EventsTable.tsx` | Tabla de eventos colapsable. Estado de colapso, filtro de severidad, búsqueda y sort son **internos**. Recibe solo `events: ApiEvent[]` (ya filtrados por fecha) y `onViewSolution`. |
...
`DashboardPage` muestra `<DbStatusBadge>` en ambos headers: el de bienvenida (inline en DashboardPage) y el principal (via `DashboardHeader`). Muestra `🟢 DB conectada` (verde) si `db_available === true`, o `🔴 DB offline · modo local` (rojo) si `false`. El badge es `null` mientras no haya respuesta del servidor. `getHealth()` falla silenciosamente y retorna `null` en caso de error.
