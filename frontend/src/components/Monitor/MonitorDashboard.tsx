...
export function MonitorDashboard({ pollingInterval = 2000 }: { pollingInterval?: number }) {
  ...
  const handleScan = async () => {
    ...
    setScanJob(null)
    try {
      const job = await triggerFleetScan(monitorClientId, monitorModels)
      setScanJob({ processed: 0, total: job.total, status: 'running' })
      
      const poll = setInterval(async () => {
        try {
          const status = await getFleetScanStatus(job.job_id)
          setScanJob({ processed: status.processed, total: status.total, status: status.status })
      ...
      <header className="dashboard__subheader" style={{ marginTop: '32px', marginBottom: '32px', padding: '24px', border: 'var(--border-glass)', background: 'var(--bg-glass)', backdropFilter: 'var(--glass-blur)' }}>
        ...
        <div className="dashboard__subheader-actions">
          ...
          <button className="dashboard__btn dashboard__btn--primary" onClick={handleScan} disabled={scanning}>
            {scanning ? (
               scanJob 
                 ? `Escaneando... ${scanJob.processed}/${scanJob.total}` 
                 : 'Iniciando...'
            ) : '🔄 Sincronizar Todo'}
          </button>
        </div>
          ...
          border: '1px solid rgba(255,255,255,0.05)' 
           ...
           <div style={{ height: '6px', background: 'rgba(255,255,255,0.05)', borderRadius: '3px', overflow: 'hidden' }}>
             <div style={{ 
               ...
               background: 'var(--hp-blue-vibrant)',
               boxShadow: '0 0 10px var(--hp-blue-vibrant)',
