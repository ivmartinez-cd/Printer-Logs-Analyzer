...
- **Advertencias de consumibles** — tabla con categoría, part number, vida útil, contador actual y estado (OK / próximo / reemplazar) basado en los códigos del log y el modelo cargado.
- **Diagnóstico con IA** — panel colapsado que llama a Claude Haiku on demand; devuelve secciones DIAGNÓSTICO / ACCIÓN / PRIORIDAD.
- **SDS Engineering Incident** — carga manual de un incidente SDS con match automático contra los códigos del log. Soporta wildcard `z` en códigos. Muestra sección "Verificar cambio de consumible" si hay solapamiento.
...
npm run test:frontend  # vitest run (70 tests)
